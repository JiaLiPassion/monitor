 ...
 * (c) 2013 Fran Peručić
 *
 * Treant is an open-source JavaScipt library for visualization of tree
 * diagrams. It implements the node positioning algorithm of John Q. Walker II
 ...
 * @constructor
 * ImageLoader is used for determening if all the images from the Tree are
 * loaded. Node size (width, height) can be correcty determined only when all
 * inner images are loaded
 */
...
ImageLoader.prototype = {
  processNode: function(node) {
    var images = node.nodeDOM.getElementsByTagName('img'), i = images.length;
    while (i--) {
 ...
 * 	Its purpose is to avoid global variables and enable multiple Trees on
 * the page.
 */
...
Tree.prototype = {
  ...
  positionTree: function(callback) {
    ...
    if (this.imageLoader.isNotLoading()) {
      ...
      if (!this.loaded) {
        this.drawArea.className +=
            ' loaded';  // nodes are hidden until .loaded class is add
  ...
  firstWalk: function(node, level) {
    ...
    if (node.childrenCount() === 0 || level == this.CONFIG.maxDepth) {
      ...
      if (leftSibling) {
        node.prelim = leftSibling.prelim + leftSibling.size() +
      ...
      if (node.stackParent) {  // hadle the parent of stacked children
        node.modifier += this.nodeDB.get(node.stackChildren[0]).size() / 2 +
  ...
  apportion: function(node, level) {
    var firstChild = node.firstChild(),
        firstChildLeftNeighbor = firstChild.leftNeighbor(), compareDepth = 1,
    ...
    while (firstChild && firstChildLeftNeighbor &&
      ...
      var modifierSumRight = 0, modifierSumLeft = 0,
          leftAncestor = firstChildLeftNeighbor, rightAncestor = firstChild;
      ...
      for (var i = 0; i < compareDepth; i++) {
        ...
        // all the stacked children are oriented towards right so use right
        // variables
      ...
      var totalGap =
          (firstChildLeftNeighbor.prelim + modifierSumLeft +
           firstChildLeftNeighbor.size() + this.CONFIG.subTeeSeparation) -
  ...
  secondWalk: function(node, level, X, Y) {
    if (level <= this.CONFIG.maxDepth) {
      var xTmp = node.prelim + X, yTmp = Y, align = this.CONFIG.nodeAlign,
          orinet = this.CONFIG.rootOrientation, levelHeight, nodesizeTmp;

      if (orinet == 'NORTH' || orinet == 'SOUTH') {
        levelHeight = this.levelMaxDim[level].height;
        nodesizeTmp = node.height;
        if (node.pseudo)
          node.height = levelHeight;  // assign a new size to pseudo nodes
      } else if (orinet == 'WEST' || orinet == 'EAST') {
        levelHeight = this.levelMaxDim[level].width;
        nodesizeTmp = node.width;
        if (node.pseudo)
          node.width = levelHeight;  // assign a new size to pseudo nodes
      }

      node.X = xTmp;

      if (node.pseudo) {  // pseudo nodes need to be properly aligned, otherwise
                          // position is not correct in some examples
        ...
        node.Y = (align == 'CENTER') ?
            ...
            (align == 'TOP') ? (yTmp + (levelHeight - nodesizeTmp)) : yTmp;
      }
      ...
      if (node.childrenCount() !== 0) {
        if (node.id === 0 && this.CONFIG.hideRootNode) {
          // ako je root node Hiden onda nemoj njegovu dijecu pomaknut po Y osi
          // za Level separation, neka ona budu na vrhu
          this.secondWalk(node.firstChild(), level + 1, X + node.modifier, Y);
        } else {
          this.secondWalk(
              node.firstChild(), level + 1, X + node.modifier,
  ...
  positionNodes: function() {
    var self = this, treeSize = {
      ...
      y: self.nodeDB.getMinMaxCoord('Y', null, null)
        ...
        // all nodes must have positive X or Y coordinates, handle this with
        // offsets
        negOffsetX =
    ...
    for (i = 0, len = this.nodeDB.db.length; i < len; i++) {
      ...
      // drawing area
      node.X += negOffsetX +
          ((treeWidth < this.drawArea.clientWidth) ? deltaX :
                                                     this.CONFIG.padding);
      node.Y += negOffsetY +
          ((treeHeight < this.drawArea.clientHeight) ? deltaY :
                                                       this.CONFIG.padding);
  ...
  handleOverflow: function(treeWidth, treeHeight) {
    ...
    if (this.CONFIG.scrollbar == 'native') {
      ...
      if (jq_drawArea.hasClass(
              'ps-container')) {  // znaci da je 'fancy' vec inicijaliziran,
                                  // treba updateat

        jq_drawArea.find('.Treant').css({width: viewWidth, height: viewHeight});
  ...
  setConnectionToParent: function(node, hidePoint) {
    var stacked = node.stackParentId, connLine,
  ...
  // create the parh which is represanted as a point, used for hideing the
  // connection
  ...
  animatePath: function(path, pathString) {
    if (path.hidden &&
        pathString.charAt(0) !== '_') {  // path will be shown, so show it
      path.show();
      path.hidden = false;
    }
    ...
    path.animate(
        ...
        this.CONFIG['animation']['connectorsAnimation'], function() {
          if (pathString.charAt(0) ===
              '_') {  // animation is hideing the path, hide it at the and of
                      // animation
            path.hide();
            path.hidden = true;
          }
  ...
  getPathString: function(from_node, to_node, stacked) {
    var startPoint = from_node.connectorPoint(true),
        ...
        connType = from_node.connStyle.type, P1 = {}, P2 = {};
    ...
    // sp, p1, pm, p2, ep == "x,y"
    var sp = startPoint.x + ',' + startPoint.y, p1 = P1.x + ',' + P1.y,
        p2 = P2.x + ',' + P2.y, ep = endPoint.x + ',' + endPoint.y,
        pm = (P1.x + P2.x) / 2 + ',' + (P1.y + P2.y) / 2, pathString,
        stackPoint;
  ...
  // algorithm works from left to right, so previous processed node will be left
  // neigbor of the next node
    ...
    if (node['children']) {
      ...
      if (node['childrenDropLevel'] && node['childrenDropLevel'] > 0) {
        while (node['childrenDropLevel']--) {
          // pseudo node needs to inherit the connection style from its parent
          // for continuous connectors
      ...
      for (var i = 0, len = node['children'].length; i < len; i++) {
        if (stack !== null) {
          newNode =
              self.createNode(node['children'][i], newNode.id, tree, stack);
          if ((i + 1) < len)
            newNode.children = [];  // last node cant have children
        } else {
...
NodeDB.prototype = {
  ...
  createNode: function(nodeStructure, parentId, tree, stackParentId) {
    var node = new TreeNode(
        nodeStructure, this.db.length, parentId, tree, stackParentId);

    this.db.push(node);
    if (parentId >= 0)
      this.get(parentId).children.push(node.id);  // skip root node
  ...
  getMinMaxCoord: function(
      dim, parent, MinMax) {  // used for getting the dimensions of the tree,
                              // dim = 'X' || 'Y'
    // looks for min and max (X and Y) within the set of nodes
    var parent = parent || this.get(0), i = parent.childrenCount(),
        MinMax = MinMax || {
...
var TreeNode = function(nodeStructure, id, parentId, tree, stackParentId) {
  ...
  // pseudo node is a node with width=height=0, it is invisible, but necessary
  // for the correct positiong of the tree
  ...
  this.connStyle =
      UTIL.createMerge(tree.CONFIG.connectors, nodeStructure['connectors']);
...
TreeNode.prototype = {
  ...
  size: function() {  // returns the width of the node
    ...
    if (this.pseudo)
      return -this.Tree().CONFIG.subTeeSeparation;  // prevents of separateing
                                                    // the subtrees
  ...
  leftMost: function(level, depth) {  // returns the leftmost child at specific
                                      // level, (initaial level = 0)
  ...
  connectorPoint: function(startPoint) {
    ...
    if (this.stackParentId) {  // return different end point if node is a
                               // stacked child
      if (orient == 'NORTH' || orient == 'SOUTH') {
        orient = 'WEST';
  ...
  pathStringThrough:
      function() {  // get the geometry of a path going through the node
  ...
  drawLineThroughMe: function(hidePoint) {  // hidepoint se proslijedjuje ako je
                                            // node sakriven zbog collapsed
    var pathString = hidePoint ? this.Tree().getPointPathString(hidePoint) :
  ...
  hide: function(collapse_to_point) {
    ...
    var jq_node = $(this.nodeDOM), tree = this.Tree(), config = tree.CONFIG,
        new_pos = {left: collapse_to_point.x, top: collapse_to_point.y};

    if (!this.hidden) {
      new_pos.width = new_pos.height = 0;
    }

    // store old width + height - padding problem when returning back to old
    // state
    if (!this.startW || !this.startH) {
      this.startW = jq_node.width();
      this.startH = jq_node.height();
    }

    // if parent was hidden in initial configuration, position the node behind
    // the parent without animations
    if (!this.positioned || this.hidden) {
      ...
      jq_node.animate(
          new_pos, config['animation']['nodeSpeed'],
          config['animation']['nodeAnimation'], function() {
    ...
    if (this.lineThroughMe) {
      ...
      if (this.hidden) {
        ...
        tree.animatePath(
            this.lineThroughMe, tree.getPointPathString(collapse_to_point));
      }
    }
  ...
  show: function() {
    ...
    var new_pos = {left: this.X, top: this.Y}, tree = this.Tree(),
        config = tree.CONFIG;
    ...
    $(this.nodeDOM)
        .animate(
            new_pos, config['animation']['nodeSpeed'],
            config['animation']['nodeAnimation'], function() {
              // $.animate applys "overflow:hidden" to the node, remove it to
...
TreeNode.prototype.createGeometry = function(tree) {
  ...
  var drawArea = tree.drawArea, image, i,

      /////////// CREATE NODE //////////////
      node = this.link.href ? document.createElement('a') :
                              document.createElement('div');

  node.className =
      (!this.pseudo) ? TreeNode.prototype.CONFIG.nodeHTMLclass : 'pseudo';
...
Tree.prototype.CONFIG = {
  ...
  'scrollbar': 'native',  // "native" || "fancy" || "None" (PS: "fancy" requires
                          // jquery and perfect-scrollbar)
...
var JSOnconfig = {
  make: function(configArray) {
    ...
    this.jsonStructure = {'chart': null, 'nodeStructure': null};
  ...
  findChildren: function(nodes) {
    ...
    while (parents.length) {
      var parentId = parents.pop(),
          ...
          i = 0, len = nodes.length, children = [];

      for (; i < len; i++) {
        var node = nodes[i];
        if (node.parent &&
            (node.parent.myID == parentId)) {  // skip config and root nodes
